...
<p>Signed in. The token is held server-side for this session.</p>
...
<form method="post" action="/logout">
  ...
  <button type="submit">Sign out</button>
        ...
        format!(
            r#"<!DOCTYPE html>
<title>Synchronizer Token</title>
<p>Anonymous. The token is held under a pre-session key.</p>
...
async fn logout(req: HttpRequest) -> actix_web::Result<HttpResponse> {
    let mut resp = HttpResponse::SeeOther();
    resp.cookie(
        Cookie::build(DEFAULT_SESSION_ID_KEY, "")
            .path("/")
...
async fn main() -> io::Result<()> {
    ...
    HttpServer::new(move || {
        let csrf = CsrfMiddlewareConfig::synchronizer_token(secret).with_secure(false);
